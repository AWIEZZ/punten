...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    fs.existsSync('./wibu.json') && zeroyt7.loadAuthInfo('./wibu.json')
    zeroyt7.on('connecting', () => {
        start('2', 'Menghubungkan...')
    })
    zeroyt7.on('open', () => {
        success('2', 'Terhubung Jangan Lupa Subscribe MBP_17')
    })
    await zeroyt7.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./wibu.json', JSON.stringify(zeroyt7.base64EncodedAuthInfo(), null, '\t'))

    zeroyt7.on('chat-update', async (message) => {
        require('./bayu.js')(zeroyt7, message, _welkom)
    })
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya Night Bot saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk meulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        but = [
          { buttonId: 'add', buttonText: { displayText: 'OKEH BG' }, type: 1 }
        ]
        sendButImage(mdata.id, wel, "JANGAN LUPA SUBSCRIBE MBP_17", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
        memeg = mdata.participants.length
        out = `Sayonara @${num.split("@")[0]}, Semoga Bahagia 😾`
        buff = await getBuffer(
        ...
        sendButImage(mdata.id, out, "BALIK LGI BAWA AYAM BAKAR YO", buff, but)
      }
...
function nocache(module, cb = () => { }) {
    console.log('Module', `'${module}'`, 'sekarang sedang diawasi bayu untuk perubahan')
